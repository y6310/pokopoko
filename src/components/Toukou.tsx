...
import { Posts, Tags, SoudanjohoType, GetPostData, GetSoudanData,PostToukou} from "../models";
...
const Toukou = () => {
    ...
    const convertToukouSoudan = (data: any) => {
      const posts: GetPostData[] = data.Posts.map((post: any) => ({
        Id: post.Id,
      ...
      // Organizationsを変換する
      const organizations: GetSoudanData[] = data.Organizations.map((org: any) => ({
        Id: org.Id,
        OrganizationName: org.OrganizationName,
        OrganizationBody: org.OrganizationBody,
        Link: org.Link,
        Tags: org.Tags.map((tag: any) => ({
          Id: tag.Id,
          TagBody: tag.TagBody,
        })),
      }));
    ...
    useEffect(() => {
      ...
      const getPostData = async () => {
        try {
          const responsePost = await axios.get<GetPostData[]>("http://localhost:8080//latest"); // レスポンスの型をGetData[]に変更
      ...
      const getSoudanjohoData = async () => {
        try {
          const responseSoudan = await axios.get<GetSoudanData[]>("http://localhost:8080//organizations");
    ...
    const postFunction = async () => {
        ...
        try {
          const response = await axios.post("http://localhost:8080//post-and-search", postData);
          const [posts]:any = convertToukouSoudan(response)
          const [organizations]:any = convertToukouSoudan(response)
          setNitamoyas(posts)
          setSoudamjohos(organizations)
