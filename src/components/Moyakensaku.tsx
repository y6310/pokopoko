...
const Moyakensaku = () => {
  ...
  useEffect(() => {
    const getToukouData = async () => {
        try {
            const response = await axios.get<GetPostData[]>("http://localhost:8080/latest"); // レスポンスの型をGetPostData[]に変更
...
const postFunction = async () => {
  ...
  try {
    const responseSoudan = await axios.post("http://localhost:8080//postsearch", postData);
    const SoudanjohosData: Posts[] = convertGetToukouDataToPosts(responseSoudan.data);
